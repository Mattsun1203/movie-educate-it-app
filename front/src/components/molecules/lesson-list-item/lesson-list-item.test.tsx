...
describe("LessonListItem", () => {
  ...
  it("statusごとにアイコンとアクセシブルなラベルが切り替わる", () => {
    const { rerender } = render(
      <LessonListItem title="環境構築" duration="8分" status="completed" />,
    );
    expect(screen.getByLabelText("視聴済み")).toHaveTextContent("✓");

    rerender(
      <LessonListItem title="環境構築" duration="8分" status="current" />,
    );
    expect(screen.getByLabelText("再生中")).toHaveTextContent("▶");

    rerender(
      <LessonListItem title="環境構築" duration="8分" status="upcoming" />,
    );
